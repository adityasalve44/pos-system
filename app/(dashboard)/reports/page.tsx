...
export default function ReportsPage() {
  ...
  const { data: daily } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/reports/daily?date=${summaryDate}`);
  ...
  // Fix 7: bulk print by date range
  async function bulkPrint() {
    ...
    if (w) {
      w.document.write(
        `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>@media print{.pb{page-break-after:always}}body{font-family:monospace}</style></head><body>${body}</body></html>`,
      );
      w.document.close();
      w.focus();
      w.print();
              ...
              <div className="flex flex-wrap gap-2">
                {Object.entries(daily.byPaymentMethod).map(
                  ([method, amount]) => (
                    <div
                      key={method}
                      className="bg-gray-50 border rounded-lg px-3 py-1.5 text-center min-w-16"
                    >
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="space-y-2 max-h-105 overflow-y-auto">
              {orders.map((order) => (
                // Fix 10: different color per type
                <div
                  key={order.id}
                ...
                >
                  ...
                  >
                    {order.orderType === "takeout" ? (
                      ...
                      <UtensilsCrossed size={14} className="text-teal-700" />
                    )}
                  </div>
                  ...
                  {/* Fix 7: view detail + print per order */}
